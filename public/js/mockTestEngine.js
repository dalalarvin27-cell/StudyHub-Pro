// ==========================================================================
// EDUVAULT - MOCK TEST ENGINE WITH TIMER & DETAILED REVIEW (RIGHT/WRONG)
// ==========================================================================
...
async function startTest(testId) {
  ...
  if (!token) {
    alert("Test start karne ke liye Login zaroori hai.");
  ...
  try {
    ...
    currentQIndex = 0;
    userAnswers = {};
    secondsRemaining = (currentTest.durationMinutes || currentQuestions.length) * 60;
...
function renderQuestion() {
  ...
  optionsContainer.innerHTML = q.options.map((opt, idx) => `
    <label style="display:flex; align-items:center; gap:12px; padding:14px 18px; border-radius:10px; border:1px solid var(--border-color); background:${selectedAns === idx ? 'var(--primary-light)' : 'var(--bg-glass)'}; cursor:pointer; transition:all 0.2s;">
  ...
  try {
    const res = await fetch(`/api/attempts/${currentTest.id}/submit`, {
      ...
      body: JSON.stringify({
        ...
        timeTakenSeconds: ((currentTest.durationMinutes || currentQuestions.length) * 60) - secondsRemaining
      })
    });
    ...
    // OPEN DETAILED RESULT & REVIEW MODAL (KONSA GALAT, KONSA SAHI)
...
/* DETAILED POST-TEST RESULT MODAL (SHOWS RIGHT / WRONG / EXPLANATION) */
        ...
        <div style="display:flex; flex-direction:column; gap:16px;">
          ${reviewData.map((rev, idx) => {
            const statusBadge = rev.isCorrect 
              ? `<span style="background:#dcfce7; color:#15803d; padding:4px 10px; border-radius:6px; font-weight:700; font-size:0.8rem;">✅ SAHI (CORRECT)</span>`
              : (rev.userAnswer === null 
                  ...
                  : `<span style="background:#fee2e2; color:#b91c1c; padding:4px 10px; border-radius:6px; font-weight:700; font-size:0.8rem;">❌ GALAT (INCORRECT)</span>`);
                  ...
                  ${rev.options.map((opt, oIdx) => {
                    ...
                    return `
                      <div style="padding:10px 12px; border-radius:8px; font-size:0.85rem; ${optStyle}">
                        ${opt} ${oIdx === rev.correctAnswer ? ' (Sahi Answer)' : ''} ${oIdx === rev.userAnswer && !rev.isCorrect ? ' (Aapka Answer)' : ''}
                      </div>
                    `;
                  }).join("")}
                </div>
